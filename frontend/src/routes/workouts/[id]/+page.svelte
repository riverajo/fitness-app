...
<div class="mx-auto max-w-3xl px-4 py-6 sm:px-6 lg:px-8">
	...
	{:else if $workoutQuery.error}
		<Alert color="red">
			<span class="font-medium">Error loading workout:</span>
	...
	{:else if $workoutQuery.data?.getWorkoutLog}
		...
		<Card class="mb-6 w-full max-w-none">
			<div class="mb-4">
				<Heading tag="h1" class="mb-2 text-2xl sm:text-3xl">{workout.name}</Heading>
		...
		<div class="space-y-6">
			...
			{#each workout.exerciseLogs as exerciseLog, i (i)}
				<Card class="w-full max-w-none overflow-hidden p-0">
					<div
						class="border-b border-gray-200 bg-gray-50 px-4 py-5 sm:px-6 dark:border-gray-600 dark:bg-gray-700"
					>
					...
					<div class="p-0">
						<Table shadow>
							...
							<TableBody>
								{#each exerciseLog.sets as set, i (i)}
									<TableBodyRow>
