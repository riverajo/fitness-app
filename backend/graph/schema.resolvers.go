...
func (r *mutationResolver) Register(ctx context.Context, input model1.RegisterInput) (*model1.AuthPayload, error) {
	...
	// 2b. Generate the Refresh token
	refreshToken, err := middleware.GenerateRefreshToken(internalUser, r.JWTSecret)
	if err != nil {
		slog.Error("CRITICAL: Failed to generate Refresh Token for new user", "user_id", internalUser.ID, "error", err)
		return nil, fmt.Errorf("registration successful, but failed to generate refresh token")
	}
...
func (r *mutationResolver) Login(ctx context.Context, input model1.LoginInput) (*model1.AuthPayload, error) {
	...
	refreshToken, err := middleware.GenerateRefreshToken(user, r.JWTSecret)
	if err != nil {
		slog.Error("CRITICAL: Failed to generate Refresh Token for user", "user_id", user.ID, "error", err)
		return nil, fmt.Errorf("internal server error during session creation")
	}

	// 3. Return successful payload with token
	return &model1.AuthPayload{
		Success:      true,
...
func (r *mutationResolver) RefreshToken(ctx context.Context, refreshToken string) (*model1.AuthPayload, error) {
	...
	claims, err := middleware.ValidateRefreshToken(refreshToken, r.JWTSecret)
	if err != nil {
		return nil, fmt.Errorf("invalid or expired refresh token")
	}

	// 2. Fetch the user to ensure they still exist and aren't banned/deleted
	...
	newToken, err := middleware.GenerateJWT(user, r.JWTSecret)
	if err != nil {
		return nil, fmt.Errorf("failed to generate new access token")
	}

	// 4. Generate new Refresh Token (Rotation)
	newRefreshToken, err := middleware.GenerateRefreshToken(user, r.JWTSecret)
	if err != nil {
		return nil, fmt.Errorf("failed to generate new refresh token")
	}
...
func (r *mutationResolver) UpdateUser(ctx context.Context, input model1.UpdateUserInput) (*model1.AuthPayload, error) {
	...
	refreshToken, err := middleware.GenerateRefreshToken(updatedUser, r.JWTSecret)
	if err != nil {
		slog.Error("CRITICAL: Failed to generate Refresh Token for updated user", "user_id", updatedUser.ID, "error", err)
		return nil, fmt.Errorf("profile updated, but failed to refresh token")
	}
